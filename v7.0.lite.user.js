// ==UserScript==
// @name         Alist 沉浸式图廊 (Immersive Gallery) v7.0 Lite
// @namespace    http://tampermonkey.net/
// @version      7.0-lite
// @description  v7.0的轻量版本，移除了所有复杂动画，专注于在性能较差的设备上提供流畅、现代的图片浏览体验。
// @author       Your Name & AI
// @match        *://127.0.0.1:5244/*
// @icon         data:image/svg+xml;base64,
// @grant        GM_addStyle
// @license      MIT
// ==/UserScript==
...
(function() {
    ...
    console.log('[Alist Gallery] Script v7.0 (Lite) is running!');
    ...
    GM_addStyle(`
        ...
        #${GALLERY_CONTAINER_ID} {
            ...
            transition: opacity 0.3s ease-in-out; /* 简化过渡 */
        ...
        /* --- 控件与按钮样式 (简化动效) --- */
        #${GALLERY_BUTTON_ID}{position:fixed;bottom:25px;right:25px;width:55px;height:55px;background:white;color:#333;border-radius:50%;border:none;cursor:pointer;z-index:9998;display:flex;justify-content:center;align-items:center;box-shadow:0 6px 20px rgba(0,0,0,.15);transition:opacity .3s, background-color .2s;opacity:0;transform:scale(0);animation:liteFadeIn .5s .2s forwards}#${GALLERY_BUTTON_ID}:hover{background-color:#f0f0f0}@keyframes liteFadeIn{to{opacity:1;transform:scale(1)}}#${GALLERY_BUTTON_ID} svg{width:28px;height:28px;color:#8ec5fc;}
        .gallery-back-btn,.gallery-toolbar{background:rgba(255,255,255,.6);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border:1px solid rgba(0,0,0,.08);color:#333;transition:background-color .2s, opacity .3s}.gallery-back-btn{position:fixed;top:20px;left:20px;width:44px;height:44px;border-radius:50%;z-index:10001;display:flex;justify-content:center;align-items:center;cursor:pointer}.gallery-back-btn:hover{background:rgba(255,255,255,.8)}.gallery-toolbar{position:fixed;top:20px;right:20px;z-index:10001;display:flex;gap:10px;padding:8px;border-radius:22px;opacity:0;visibility:hidden;transform:translateY(-10px);transition-property:opacity,visibility,transform}#${GALLERY_CONTAINER_ID}:hover .gallery-toolbar{opacity:1;visibility:visible;transform:translateY(0)}.toolbar-btn{width:36px;height:36px;border:none;background:transparent;color:#333;cursor:pointer;border-radius:50%;display:flex;justify-content:center;align-items:center;transition:background-color .2s, color .2s}.toolbar-btn:hover{background:rgba(0,0,0,.05)}.toolbar-btn.active{background:#8ec5fc;color:#fff !important}
        ...
        .gallery-card {
            ...
            /* 简化入场动画 */
            opacity: 0;
            transition: opacity 0.5s ease, aspect-ratio 0.6s cubic-bezier(0.4, 0, 0.2, 1), border-radius 0.4s ease;
            aspect-ratio: 3 / 4;
            min-height: 300px;
        }
        .gallery-card.is-visible { opacity: 1; }
        .card-placeholder { position: absolute; top: 0; left: 0; width: 100%; height: 100%; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); display: flex; justify-content: center; align-items: center; color: rgba(0,0,0,0.4); font-family: sans-serif; font-size: 1.2em; transition: opacity 0.5s; }
        
        /* --- 简化图片加载效果 --- */
        .gallery-image-wrapper { position: absolute; top: 0; left: 0; width: 100%; height: 100%; }
        .gallery-image { display: block; width: 100%; height: 100%; opacity: 0; transition: opacity 0.5s ease-in-out; object-fit: contain; }
        ...
        .gallery-image-list.mode-webtoon .card-filename { display: none; }
            ...
            for (const entry of entries) {
                if (entry.isIntersecting) {
                    ...
                    try {
                        ...
                        finalImage.onload = () => {
                            // 1. 动态调整比例 (保留平滑过渡)
                            card.style.aspectRatio = finalImage.naturalWidth / finalImage.naturalHeight;
