// ==UserScript==
// @name         Alist Ultimate Media Experience v11.0
// @namespace    http://tampermonkey.net/
// @version      11.0
// @description  v11.0 "Architect" - Major architectural refactor into distinct modules for enhanced stability and maintainability. Introduces a sophisticated, silky-smooth loading animation for all images and an intelligent lazy-loading mechanism that prevents unnecessary requests during fast scrolling.
// @author       Your Name & AI
// @license      MIT
...
(function () {
    ...
    console.log('[Alist Ultimate Media] Script v11.0 (Architect) is running!');

    // --- 全局常量与配置 ---
    const DARK_MODE_STORAGE_KEY = 'alist_ultimate_dark_mode_v11.0';
    ...
    const PosterWallModule = {
        ...
        CONFIG: {
            ...
            CACHE_KEY_PREFIX: 'alist_poster_cache_v11.0_',
        ...
        createView() {
            ...
            const title = document.createElement('h1'); title.className = 'poster-wall-title'; this.elements.galleryTitle = title; header.appendChild(title);
            const grid = document.createElement('div'); grid.className = 'poster-wall-grid'; this.elements.galleryGrid = grid;
            const closeBtn = document.createElement('button'); closeBtn.className = 'poster-wall-close-btn'; closeBtn.innerHTML = '&times;'; closeBtn.onclick = () => this.close(); this.elements.galleryCloseBtn = closeBtn;
            const darkModeBtn = document.createElement('button'); darkModeBtn.id = 'dark-mode-toggle-btn'; darkModeBtn.title = '切换背景模式'; darkModeBtn.innerHTML = `<svg class="icon-moon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg><svg class="icon-sun" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>`;
            darkModeBtn.onclick = toggleDarkMode; this.elements.darkModeToggleButton = darkModeBtn;
            const pagination = document.createElement('div'); pagination.className = 'poster-pagination-controls'; pagination.innerHTML = `<button class="pagination-btn prev-btn">&lt; 上一页</button><span class="pagination-info"></span><button class="pagination-btn next-btn">下一页 &gt;</button>`;
            pagination.querySelector('.prev-btn').onclick = () => this.goToPrevPage();
            pagination.querySelector('.next-btn').onclick = () => this.goToNextPage();
            this.elements.paginationControls = pagination;
            container.append(closeBtn, darkModeBtn, header, grid, pagination);
            document.body.appendChild(container); this.elements.galleryContainer = container;
            applyDarkModeState();
        },

        open() {
            const breadcrumbItems = document.querySelectorAll('.hope-breadcrumb__list .hope-breadcrumb__item');
            const lastItem = breadcrumbItems[breadcrumbItems.length - 1]?.querySelector('.hope-breadcrumb__link');
            this.elements.galleryTitle.textContent = lastItem ? lastItem.textContent : 'Gallery';
            document.body.classList.add('poster-wall-is-active');
            document.querySelector('.obj-box').style.display = 'none';
            this.elements.galleryContainer.style.display = 'block';
            this.elements.galleryContainer.scrollTop = 0;
        ...
        close() {
            document.body.classList.remove('poster-wall-is-active');
            this.elements.galleryContainer.style.display = 'none';
            document.querySelector('.obj-box').style.display = '';
            this.initializeState();
        },

        buildAndLaunch() {
            const items = Array.from(document.querySelectorAll('.obj-box .grid-item, .obj-box .list-item'));
            this.state.allItems = items.filter(item => !!item.querySelector('svg [d^="M496 152a56"], svg [d^="M464 128H272l-54.63-54.63"]'));
            this.state.totalPages = Math.ceil(this.state.allItems.length / this.CONFIG.PAGE_SIZE);
        ...
        renderCurrentPage() {
            ...
            pageItems.forEach(item => {
                ...
                const cardContainer = document.createElement('div'); cardContainer.className = 'poster-card-container is-skeleton';
                ...
                this.state.cards.set(path, task);
        ...
        updatePaginationControls() {
            ...
            if (this.state.totalPages <= 1) { this.elements.paginationControls.style.display = 'none'; return; }
            this.elements.paginationControls.style.display = 'flex';
            const [prevBtn, info, nextBtn] = this.elements.paginationControls.children;
            info.textContent = `第 ${this.state.currentPage} / ${this.state.totalPages} 页`;
        ...
        setupLazyLoading() {
            this.state.observer = new IntersectionObserver(entries => {
                entries.forEach(entry => {
                    const timer = this.state.lazyLoadTimers.get(entry.target);
                    clearTimeout(timer);
                    if (entry.isIntersecting) {
                        const newTimer = setTimeout(() => {
                            const path = entry.target.dataset.path; if (!path) return;
                            const task = this.state.cards.get(path);
                            if (task && task.state === 'idle') this.processQueue(task);
                        }, this.CONFIG.LAZY_LOAD_DELAY);
                        this.state.lazyLoadTimers.set(entry.target, newTimer);
                    }
                });
        ...
        processQueue(task) {
            if (this.state.activeScans >= this.CONFIG.MAX_CONCURRENT_SCANS) {
                setTimeout(() => this.processQueue(task), 200);
                return;
            }
            this.transitionState(task, 'SCAN');
        },

        transitionState(task, action) {
            if (!task || !task.el) return;
            switch (task.state) {
                case 'idle': if (action === 'SCAN') { task.state = 'scanning'; this.scanAndProcessFolder(task); } break;
                case 'scanning':
                    if (action === 'PROCESS_SUCCESS') { task.state = 'done'; this.renderCard(task, false); }
                    else if (action === 'NO_COVER_FOUND' || action === 'PROCESS_ERROR') { task.state = 'done_no_cover'; this.renderCard(task, true); }
                    break;
            }
        },
        ...
        async scanAndProcessFolder(task) {
            ...
            try {
                ...
                if (cachedData) {
                    if (cachedData.noCover) { this.transitionState(task, 'NO_COVER_FOUND'); } else {
                        const blob = await (await fetch(cachedData.signedUrl, { signal })).blob();
                        task.imageBlobUrl = URL.createObjectURL(blob); this.transitionState(task, 'PROCESS_SUCCESS');
                    } return;
                }
                ...
                if (!linkResp.ok) {
                    ...
                    sessionStorage.setItem(cacheKey, JSON.stringify({ noCover: true })); this.transitionState(task, 'NO_COVER_FOUND'); return;
                }
                ...
                this.transitionState(task, 'PROCESS_SUCCESS');
            } catch (error) { if (error.name !== 'AbortError') { console.error(`[Poster Wall] Process failed for ${task.path}:`, error); this.transitionState(task, 'PROCESS_ERROR'); } }
            finally { this.state.activeScans--; }
        },

        renderCard(task, isFallback) {
            ...
            if (isFallback) {
                ...
                if (match) {
                    const [ , author, title ] = match.map(s => s.trim());
            ...
            this.addInteractiveEffects(containerElement);
        },

        addInteractiveEffects(containerElement) {
            if (Utils.isTouchDevice()) return; const MAX_TILT = 15; let animationFrameId = null;
            containerElement.addEventListener('mousemove', (e) => {
                if (animationFrameId) cancelAnimationFrame(animationFrameId);
                animationFrameId = requestAnimationFrame(() => {
                    const rect = containerElement.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top;
                    const rotateY = (x / rect.width - 0.5) * 2 * MAX_TILT; const rotateX = (0.5 - y / rect.height) * 2 * MAX_TILT;
    ...
    const ImmersiveGalleryModule = {
        // --- 配置 ---
        CONFIG: {
            IMAGE_EXTENSIONS: ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', 'avif', '.svg', '.JPG', '.jxl', '.JXL'],
            TRIGGER_IMAGE_COUNT: 5,
            API_LIST_ENDPOINT: '/api/fs/list',
            API_LINK_ENDPOINT: '/api/fs/link',
            API_GET_ENDPOINT: '/api/fs/get',
            LARGE_FILE_THRESHOLD_MB: 15,
            MAX_CONCURRENT_DOWNLOADS: 3,
            LOAD_RETRY_COUNT: 2,
            CONTAINER_ID: 'immersive-gallery-container',
        ...
        // --- 方法 ---
        async launchForPath(path, fallbackHref) {
            const token = localStorage.getItem('token');
            if (!token) {
                alert('无法获取认证Token，请刷新页面后重试。');
                window.location.href = fallbackHref; return;
            }
            const loader = document.createElement('div'); loader.className = 'gallery-global-loader';
            loader.innerHTML = `<svg width="48" height="48" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">${this.CONFIG.ICONS.LOADER}</svg>`;
            document.body.appendChild(loader);
            try {
                const listResp = await fetch(this.CONFIG.API_LIST_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: token }, body: JSON.stringify({ path: path, page: 1, per_page: 0 }) });
                if (!listResp.ok) throw new Error(`Failed to list files: ${listResp.status}`);
                const listData = await listResp.json();
                const files = listData.data.content || [];
                this.state.imageList = files.map(file => {
                    const isImage = !file.is_dir && this.CONFIG.IMAGE_EXTENSIONS.some(ext => file.name.toLowerCase().endsWith(ext.toLowerCase()));
                    return isImage ? { name: file.name, path: `${path.endsWith('/') ? path : path + '/'}${file.name}` } : null;
                }).filter(Boolean);

                if (this.state.imageList.length < this.CONFIG.TRIGGER_IMAGE_COUNT) {
                    console.log(`[Gallery] Not enough images (${this.state.imageList.length}). Navigating to folder.`);
                    window.location.href = fallbackHref; return;
                }
                await this.launch();
            } catch (error) {
            ...
            for (const image of this.state.imageList) {
                const p = (async () => {
                    try {
                        ...
                        image.size = fileData.size; image.width = fileData.width; image.height = fileData.height;
                ...
                promises.push(p);
                if (CONCURRENT_LIMIT <= this.state.imageList.length) { const e = p.finally(() => executing.splice(executing.indexOf(e), 1)); executing.push(e); if (executing.length >= CONCURRENT_LIMIT) await Promise.race(executing); }
            }
            await Promise.all(promises);
        },

        close() {
            if (!this.state.isActive) return; if (this.state.controller) this.state.controller.abort();
            if (document.fullscreenElement) document.exitFullscreen().catch(e => console.warn(e));
            const container = document.getElementById(this.CONFIG.CONTAINER_ID); if (!container) return;
            const scrollContainer = container.querySelector(".gallery-scroll-container");
            if (scrollContainer) {
                if (this.state.clickHandler) scrollContainer.removeEventListener('click', this.state.clickHandler);
                if (this.state.progressScrollHandler) scrollContainer.removeEventListener('scroll', this.state.progressScrollHandler);
                if (scrollContainer.reprioritizeListener) scrollContainer.removeEventListener('scroll', scrollContainer.reprioritizeListener);
            }
            this.updateViewportMeta(false); clearTimeout(this.state.hideControlsTimeout);
            this.downloadManager.queue = []; this.state.isActive = false;
            container.classList.remove("gallery-active");
            container.addEventListener("transitionend", () => { container.remove(); document.body.classList.remove('gallery-is-active'); }, { once: true });
            document.removeEventListener("keydown", this.handleKeyPress);
            if (this.state.loadObserver) this.state.loadObserver.disconnect();
        },
    ...
    function applyDarkModeState() {
        if (PosterWallModule.elements.galleryContainer) {
            PosterWallModule.elements.galleryContainer.classList.toggle('dark-mode-active', isDarkModeActive);
        }
        if (PosterWallModule.elements.darkModeToggleButton) {
            PosterWallModule.elements.darkModeToggleButton.classList.toggle('dark-active', isDarkModeActive);
        }
    }
    function initialize() {
